...
static int num_thread = 2;     // amount of thread
...
{
  int i;
  for (i = 0; i < num_trans/num_thread; i++) {
...
int main(int argc, char **argv)
{
  ...
  if (!(argc == 1 || argc == 3 || argc == 5 || argc == 7 || argc == 9)) {
    fprintf(stderr, "usage: %s [-a <accts>] [-i <transactions>] [-t <think-time>] [-n <number-of-threads>]\n", argv[0]);
    exit(-1);
  }
  ...
  for (i = 0; i < num_thread; i++) pthread_create(&thread[i], NULL, threadtrans, NULL);
  for (i = 0; i < num_thread; i++) pthread_join(thread[i], NULL);
