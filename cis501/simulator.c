  ...
  while (true) {
    ...
    if (microOpCount == 1) {
      totalMacroops++;
			 ...
			 index=addressForMemoryOp%64;
			 tag=addressForMemoryOp>>=6;
		...
		if(sr1_ready && sr2_ready &&dr_ready && flag_ready){
		    if(loadStore == 'L'){
			     if(hit)flag[destinationRegister] = 2;
			     else flag[destinationRegister]=9;
			}
